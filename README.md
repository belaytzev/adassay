...
cat page.html | adfilter                      # из stdin: ни локальной, ни общей базы
...
```sh
adfilter vote https://example.com/article --ad     # всё, что фильтр не оставил на странице
adfilter vote <sha256-сегмента> --not-ad           # один сегмент по хешу
adfilter vote "точный текст абзаца" --ad           # то же, хеш считается на месте
```
...
у `vote`: `--share` (адрес общей базы поверх `$ADFILTER_SHARE_URL`), `--db` и `--config`.
...
Флаги `adfilter`: `--config` (свой `rules.yaml`), `--db` (путь к локальной базе),
`--no-share` (ничего не отправлять), `--json`, `--verbose`.

Подбор порогов по размеченному корпусу — `adfilter calibrate` (`--corpus`, по умолчанию
...
`adfilter-mcp`: `--config` и `--db`, транспорт — stdio.
...
- **Один запрос k-анонимен, страница целиком — нет.** Бакет запрашивается на каждый
  ...
  ложных — до тех пор `ADFILTER_SHARE_URL` стоит задавать только тому серверу, которому
...
Настройки: `--addr` (по умолчанию `:8080`), `--db` / `ADFILTER_SERVER_DB` (по умолчанию
`adfilter-server.db` в рабочем каталоге), `--trusted-proxies` / `ADFILTER_TRUSTED_PROXIES`.
...
**Метрики:** `adfilter_requests_total{endpoint}`, `adfilter_bucket_hits_total`,
`adfilter_submit_divergent_total`, `adfilter_verdicts_published`,
`adfilter_verdicts_quarantined`. Счётчики базы обновляются не чаще раза в 30 секунд.
